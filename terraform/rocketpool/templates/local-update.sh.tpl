...
export POOL_NAME="${rocketpool_pool}"
...
-v    | --version      The version of Rocketpool Smartnode to install
EOF
}
...
if [[ -z "$$VERSION" ]]; then
  echo "ERROR: Rocketpool version must be set."
  usage
  exit 1
fi
...
pushd $${SCRIPT_DIR}/../ansible
ansible-playbook local-update.yml -e "POOL=$${POOL_NAME}" -e "VERSION=$${VERSION}" "$$SKIP_TAGS_ARG"
popd
